...
@discardableResult
func makeClientGETBenchmark() -> Benchmark? {
...
func makeClient20ConcurrentGETBenchmark() -> Benchmark? {
    ...
    let concurrency = 20
    return Benchmark("Client: GET benchmark 20 concurrent connections", configuration: .init(metrics: defaultMetrics, scalingFactor: .kilo)) { benchmark in
        let port = serverMutex.withLock{ $0 }!.localAddress!.port!
        let logger = Logger(label: "test")
