...
public struct HSETEX<Field: RESPStringRenderable, Value: RESPStringRenderable>: ValkeyCommand {
    ...
    @inlinable public func encode(into commandEncoder: inout ValkeyCommandEncoder) {
        commandEncoder.encodeArray("HSETEX", key, keyCondition, fieldsCondition, expiration, RESPArrayWithTokenAndCount("FIELDS", fieldsData))
    }
}
...
extension ValkeyClientProtocol {
    ...
    /// - Returns: Map of fields and their values stored in the hash, or an empty list when key does not exist. In RESP2 this is returned as a flat array.
    @inlinable
    ...
    /// - Available: 2.0.0
    /// - Complexity: O(N) where N is the size of the hash.
    /// - Returns: List of fields in the hash, or an empty list when the key does not exist.
    @inlinable
    ...
    /// - Available: 2.0.0
    /// - Complexity: O(1)
    /// - Returns: Number of the fields in the hash, or 0 when the key does not exist.
    @inlinable
    ...
    public func hsetex<Field: RESPStringRenderable, Value: RESPStringRenderable>(
        ...
        try await execute(HSETEX(key, keyCondition: keyCondition, fieldsCondition: fieldsCondition, expiration: expiration, fieldsData: fieldsData))
    }
    ...
    /// - Available: 3.2.0
    /// - Complexity: O(1)
    /// - Returns: String length of the value associated with the field, or zero when the field is not present in the hash or key does not exist at all.
    @inlinable
    ...
    /// - Available: 2.0.0
    /// - Complexity: O(N) where N is the size of the hash.
    /// - Returns: List of values in the hash, or an empty list when the key does not exist.
    @inlinable
