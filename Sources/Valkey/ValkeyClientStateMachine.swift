...
> where ConnectionPoolFactory.ConnectionPool == ConnectionPool {
    ...
    mutating func addReplicas(nodeIDs: [ValkeyNodeID]) -> AddReplicasAction {
        var nodes = [ValkeyNodeDescription(endpoint: self.nodes.primary.endpoint, port: self.nodes.primary.port)]
        nodes.append(contentsOf: nodeIDs.lazy.map { .init(endpoint: $0.endpoint, port: $0.port) })
