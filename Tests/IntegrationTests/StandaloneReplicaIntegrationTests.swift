...
struct StandaloneReplicaIntegrationTests {
    ...
    @Test func testFailover() async throws {
        var logger = Logger(label: "testFailover")
        logger.logLevel = .trace
        try await withValkeyClient(.hostname("localhost", port: 36001), configuration: .init(), logger: logger) { client in
            try await withKey(client) { key in
                try await client.failover()
                try await client.set(key, value: "FAILOVER")
            }
        }
    }
