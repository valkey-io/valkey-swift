...
struct ClientIntegratedTests {
    ...
    func testClientSetGet() async throws {
        ...
        try await withValkeyClient(.hostname(valkeyHostname, port: 6379), logger: logger) { valkeyClient in
            ...
            let response = try await valkeyClient.get("sdf").map { String(fromBulkString: $0) }
            #expect(response == "Hello")
            let response2 = try await valkeyClient.get("sdf65fsdf").map { String(fromBulkString: $0) }
            #expect(response2 == nil)
        }
    }
    ...
    func testPipelinedSetGet() async throws {
        ...
        try await withValkeyConnection(.hostname(valkeyHostname, port: 6379), logger: logger) { connection in
            try await withKey(connection: connection) { key in
                ...
                try #expect(responses.1.get().map { String(fromBulkString: $0) } == "Pipelined Hello")
            }
        }
    }
    ...
    func testTransactionSetIncrGet() async throws {
        ...
        try await withValkeyClient(.hostname(valkeyHostname, port: 6379), logger: logger) { client in
            try await withKey(connection: client) { key in
                ...
                #expect(try responses.2.get().map { String(fromBulkString: $0) } == "101")
            }
        }
    }
    ...
    func testInvalidTransactionSetIncrGet() async throws {
        ...
        try await withValkeyClient(.hostname(valkeyHostname, port: 6379), logger: logger) { client in
            try await withKey(connection: client) { key in
                ...
                let result = try responses.2.get().map { String(fromBulkString: $0) }
                #expect(result == "101")
            }
        }
    }
                ...
                for _ in 0..<100 {
                    group.addTask {
                        try await withKey(connection: connection) { key in
                            ...
                            let response = try await connection.get(key).map { ValkeyKey(fromBulkString: $0) }
                            #expect(response == key)
                        }
                    }
                }
                ...
                try await withKey(connection: connection) { key in
                    ...
                    for _ in 0..<100 {
                        group.addTask {
                            ...
                            try #expect(responses.1.get().map { String(fromBulkString: $0) } == value)
                        }
                    }
                }
                try await group.waitForAll()
    ...
    func testAuthentication() async throws {
        ...
        try await withValkeyConnection(
            ...
            #expect(String(fromBulkString: user) == "johnsmith")
        }
    }
            ...
            try await withThrowingTaskGroup(of: Void.self) { group in
                ...
                try await withThrowingTaskGroup(of: Void.self) { group in
                    ...
                    try await valkeyClient.withConnection { connection in
                        let value = try await connection.get(key).map { String(fromBulkString: $0) }
                        #expect(value == "1000")
    ...
    func testClientInfo() async throws {
        ...
        logger.logLevel = .trace
        try await withValkeyClient(.hostname(valkeyHostname, port: 6379), logger: logger) { client in
            let clients = String(fromBulkString: try await client.clientList())
    ...
    func testMultipleDB() async throws {
        ...
        for dbNum in 0...15 {
            ...
            try await withValkeyConnection(.hostname(valkeyHostname, port: 6379), configuration: clientConfig, logger: logger) { connection in
                ...
                let clientInfo = String(fromBulkString: try await connection.clientInfo())
                ...
                let response = try await connection.get(ValkeyKey(key)).map { String(fromBulkString: $0) }
                ...
                for otherDbNum in 0...15 {
                    ...
                    let otherResponse = try await connection.get(ValkeyKey(otherKey)).map { String(fromBulkString: $0) }
