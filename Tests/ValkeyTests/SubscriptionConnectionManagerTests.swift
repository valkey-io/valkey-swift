...
struct SubscriptionConnectionManagerTests {
    ...
    func setupConnection() async throws -> ValkeyConnection {
        let channel = NIOAsyncTestingChannel()
        let connection = try await ValkeyConnection.setupChannelAndConnect(channel, configuration: .init(), logger: Logger(label: "test"))
            ...
            for _ in 0..<500 {
                group.addTask {
                    let connection = try await referencedObject.value.acquire {
                        ...
                        return try await self.setupConnection()
                    }
                    let connection2 = try await referencedObject.value.acquire {
                        try await Task.sleep(for: .milliseconds(50))
                        return try await self.setupConnection()
